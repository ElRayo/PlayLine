...
void QTRSensors::setTypeAnalogMUX()
{
  _type = QTRType::AnalogMUX;
  _maxValue = 1023; // Arduino analogRead() returns a 10-bit value by default
}
...
void QTRSensors::setSensorPins(const uint8_t * pins, uint8_t sensorCount)
{
  ...
  // If Analog MUX sensor, the input is Pins[0] "Unique input"
  if(_type == QTRType::AnalogMUX)
  {
	  _sensorPins[0] = pins[0];
  }
  // If point to point sensor map all sensors
  else
  {
...
// Configuration of selector pins 
void QTRSensors::setSelectorPins(const uint8_t * selectPins, uint8_t selectCount)
{
  // (Re)allocate and initialize the array if necessary.
  uint8_t * oldSelectorPins = _selectPins;
  _selectPins = (uint8_t *)realloc(_selectPins, sizeof(uint8_t) * selectCount);
  if (_selectPins == nullptr)
  {
    // Memory allocation failed; don't continue.
    free(oldSelectorPins); // deallocate any memory used by old array
    return;
  }
  //Set selector pins as output, map the selectors and number of selectors
...
void QTRSensors::setEnablePins(const uint8_t * enablePins, uint8_t enableCount)
{
  // (Re)allocate and initialize the array if necessary.
  uint8_t * oldEnablePins = _enablePins;
  _enablePins = (uint8_t *)realloc(_enablePins, sizeof(uint8_t) * enableCount);
  if (_enablePins == nullptr)
  {
    // Memory allocation failed; don't continue.
    free(oldEnablePins); // deallocate any memory used by old array
    return;
  }
  //Set selector pins as output, map the enables and number of enables
...
void QTRSensors::readPrivate(uint16_t * sensorValues, uint8_t start, uint8_t step)
{
  ...
  switch (_type)
  {
    ...
    case QTRType::AnalogMUX:
      // reset the values
      for (uint8_t i = start; i < _sensorCount; i += step)
      {
        sensorValues[i] = 0;
      }
	//SET THE MUX TO FLOW DATA
	for (uint8_t k = 0; k < _samplesPerSensor; k ++)
    {
        for(uint8_t j = 0; j < _sensorCount; j++)
        {
			for (uint8_t i = 0; i < _selectCount; i += step)
			{
				digitalWrite(_selectPins[i], (j>>i)%2);
			}
			
			//In this version only have to one or two strips
			if(_sensorCount>8)
			{
				if(j < 8)
				{
					digitalWrite(_enablePins[0], LOW);
					digitalWrite(_enablePins[1], HIGH);
				}
				else
				{
					digitalWrite(_enablePins[0], HIGH);
					digitalWrite(_enablePins[1], LOW);
				}
			}
			else
			{
            ...
            sensorValues[j] += analogRead(_sensorPins[0]);//analogRead(A0);   // add the conversion result
        }
    }
